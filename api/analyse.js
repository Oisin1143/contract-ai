...
export default async function handler(req, res) {
  ...
  if (!contractText || !disputeDesc) {
    return res
      .status(400)
      .json({ error: "contractText and disputeDesc are required." });
  }
  if (typeof contractText !== "string" || typeof disputeDesc !== "string") {
    return res.status(400).json({ error: "Invalid input types." });
  }
  // Basic length caps — prevents abuse & keeps Groq costs bounded
  if (contractText.length > 50000) {
    return res
      .status(400)
      .json({ error: "Contract text too long (max 50,000 characters)." });
  }
